...
(function(exports) {
  ...
  exports.create = function() {
	...
	*/ 
    ...
    .route(renditions.appSystemRoutes);
  ...
  /* App Main Class */
	...
	//Routing - A route is bound to a Resource which inherits from ResourceWrapper Object
	...
	* Directories are usually handled via system Lua scripts.
	* The 'h' is an internal value that represents the '#' hash 
	* symbol as part of the path.
	*/
	...
	*/
	...
	App._sysModules = []; // This was used for adding external modules, may not need
	App._regpathParentName = /\w+(?=\/)/;
	App._regSelector = /\/{0}(?:@)[a-zA-Z0-9\-\(\)\|]+\/{0}/; // may not need this was for movie/(staff|director)
	...
	App.response = function(serverMode) {
		...
		return function (request, response) {
			...
			*/ 
			...
			// Executes the Handler that is configured for the Route, a ClientMessage object is created	
			routeObject.exec(response);
		}; 
	};
	
	// Append the new routes to the master App.routes object
	App.applyRoutes = function(routes) {
