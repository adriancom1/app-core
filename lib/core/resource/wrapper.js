...
var ResourceResolver = require('./resolver');
...
function ResourceWrapper() {
	//this.values = []; // may not need
	this._isService = false;
	this.urlmap = null;

	// Default Redis record hashmap table key (ie.record:id:details)
	// TODO: Can be abstracted into a separate module, the bottom 2 constants
	this.DEFAULT_HASH_TABLE = 'details'; //m
	this.DEFAULT_CONTENT_RESOURCE = 'page'; //m

	//Resource, Route Paths and URL Properties
	// TODO: Add a resolver for this to return a map
	var resRouteData = arguments[0];
	console.log('resPaths===', resRouteData);

	// SpecialRules parameter <name>
	var hasSpecialRules = this.hasRules = arguments[0][2]; //resRouteData[2]
	console.log("SpecialRules= ", arguments);
	...
	try {
		...
		if (arguments.length == 0) {
			throw new Error("ResourceWrapper():: cannot be instantiated.");
		}

		ResourceWrapper.create.call(this, resRouteData, hasSpecialRules);
...
ResourceWrapper.create = function(resourceRoutePaths, isSpecialRules) {
	
	if (arguments.length == 0) {
		throw new Error("ResourceWrapper.create():: Cannot instantiate");
	}
    ...
    */
	if (ext) {
		...
		console.log("Extension=", this.type);
	};
...
ResourceWrapper.prototype = {
	 ...
	 * This should return the Resource wrapped by this instance.
	 */
	getResource: function() {
		...
		// TODO: do this only once and cache it for future requests
		...
		*/
		...
		try {
			...
			if (umap.script && scriptName !== this.DEFAULT_HASH_TABLE) {
				...
				// argv is a special Redis argument table value
				var argv = resKeys[2];
				/**
		...
		*/
		...
		if (this.isParent() != false) { //TODO:: Handle if a Summary hashkey was sent instead
			...
			*/
			...
			if (vParent === hashkey) {
				hashkey = this.DEFAULT_HASH_TABLE;
			}
			...
			if (script.hasOwnProperty(hashkey) == false && isScript == true) {
				...
				/* 
					if a 'mode' was implicitly set & an internal LUA script config was
	...
	*/
	...
	/*
		...
		TODO: Create the ValueMap Object or syntehtic resource
	*/
	...
	*/
	...
	output: function(output) {
		// TODO: need to investigate this function further. not sure if its still needed.
		var self = this;
		console.log("MAP Output==", this.type);
		var rendition = (this.type) ? this.type : (function(value) {
			//console.log("THIS IS THE ResourceWrapper Output MESSAGE======,", value);
			//test value
			//Todo: Fix this.
	...
	render: function(renditions) {
		...
		// TODO: Abstract the 'post' render as a module/ may not need
...
$S.extend(ResourceWrapper.prototype, EventEmitter.prototype);
//$S.extend(ResourceWrapper.prototype, ResourceResolver.prototype);
